  ...
  const [counts, setCounts] = useState<Counts>({
    ...
    const fetchCounts = async () => {
      const [patients, appointments, insurances] = await Promise.all([
        fetch(`${BASE_URL}/ai-intake/patients/count`).then((r) => r.json()),
        fetch(`${BASE_URL}/ai-intake/appointments/count`).then((r) => r.json()),
        fetch(`${BASE_URL}/ai-intake/insurances/count`).then((r) => r.json()),
      ]);
